...
def build_concepts_blocks(ad_concepts, session_id, style_gallery=None):
    ...
    for i, concept in enumerate(ad_concepts):
        ...
        headline = concept.get('headline', f'Concept #{i+1}')
        subheadline = concept.get('subheadline', '')
        angle = concept.get('ad_angle', 'Creative')
        visuals = concept.get('visual_direction', 'No Visual Direction provided.')
        style = concept.get('style_category', 'STUDIO').upper()
        reasoning = concept.get('design_reasoning', 'Strategic layout for high conversion.')
