...
def generate_fal_flux_image(concept_json, product_image_path="bottle.png", style_image_path=None):
    ...
    """
    # Fallback chain for the prompt
    nano_prompt = concept_json.get('nano_banana_prompt', '').strip()
    if not nano_prompt:
        nano_prompt = concept_json.get('visual_direction', '').strip()
        
    if not nano_prompt:
        logging.error(f"Blueprint JSON missing prompt keys. Keys found: {list(concept_json.keys())}")
        raise ValueError("No prompt found in the blueprint JSON (checked 'nano_banana_prompt' and 'visual_direction').")
