...
import anthropic
import requests
from extract_context import extract_urls, scrape_product_url, get_brand_guidelines
...
### OUTPUT RULES:
1. You MUST output your response STRICTLY as a JSON array containing exactly 3 JSON objects.
2. Each JSON object MUST have exactly these 7 keys:
   - "headline": The main ad headline (max 10 words).
   - "subheadline": Supporting copy/tagline.
   - "ad_angle": The marketing angle (e.g., 'Performance-Focused', 'Luxury-Studio').
   - "visual_direction": High-level description for the user in Slack.
   - "nano_banana_prompt": Detailed spatial and technical prompt for Fal.ai (Nano-Banana 2) following the Narrative Prompt Template.
   - "style_category": One of: [STUDIO, LIFESTYLE, MINIMAL].
   - "design_reasoning": Brief explanation of why this design converts.
"""
...
2. The JSON object must have exactly these 7 keys: headline, subheadline, ad_angle, visual_direction, nano_banana_prompt, style_category, design_reasoning.
"""
