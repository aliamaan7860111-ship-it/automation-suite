...
- Notion **Leads DB** (ID from `LEADS_DB_ID` env) — CRM.
- Notion **Agent Roster DB** (ID from `ROSTER_DB_ID` env) — agents, IDs, shifts, active flag.

## The 5 brands (canonical ALL-CAPS)

Brand identifiers redacted in this public showcase. The live deployment maps each
`whatsapp_bot_name` → `phone_number_id` → canonical brand label via JSON env vars
(`BRAND_PHONE_MAP_JSON`, `BRAND_BOT_MAP_JSON`).

| whatsapp_bot_name (webhook) | phone_number_id (API) | Canonical brand |
|---|---|---|
| Brand 1 (any case) | `<redacted>` | BRAND_1 |
| Brand 2 (any case) | `<redacted>` | BRAND_2 |
| Brand 3 (any case) | `<redacted>` | BRAND_3 |
| Brand 4 (any case) | `<redacted>` | BRAND_4 |
| Brand 5 (any case) | `<redacted>` | BRAND_5 |
...
- Filter:
  - `agent_name` ∈ roster active names (loaded from Roster DB at runtime) → real agent reply → apply Rule 3.
  - Else (admin, bot, automation, off-roster sessions) → **ignore entirely**. Do not stamp anything.
- As a side-effect, sync labels (see Rule 4).
...
- **Batch safety:** hold the pointer in memory during processing. After assigning agent N, the next assignment in the same process is agent N+1, regardless of Notion propagation lag.
...
- Incoming: `http://VM_IP:8082/rpgrq/incoming`
- Outgoing: `http://VM_IP:8082/rpgrq/outgoing`
