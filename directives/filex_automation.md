...
Filex pushes status updates to `http://VM_IP:8080/filex_webhook`. The bot maps each status into the `FILEX STATUS` select in Notion and updates `Last Update`. No team action required.
